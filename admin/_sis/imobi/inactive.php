...
<header class="dashboard_header">
    ...
    <div class="dashboard_header_search">
        <form name="searchImobi" action="" method="post" enctype="multipart/form-data" class="ajax_off">
            ...
            <button title="Pesquisar" class="btn_header btn_darkblue icon icon-search icon-notext"></button>
        </form>
    </div>
...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()):
        ...
        echo Erro("<span class='al_center icon-notification'>Parabéns {$Admin['user_name']}, Não Existem Imóveis Indisponíveis!</span>", E_USER_NOTICE);
    else:
        foreach ($Read->getResult() as $REALTY):
            ...
            echo "<article class='box box25 imobi_single' id='{$realty_id}'>           
                <div class='post_single_cover'>
                    ...
                    <div class='post_single_status'><span class='btn btn_grey'>" . str_pad($realty_views, 4, 0, STR_PAD_LEFT) . "</span>{$RealTyStatus}</div>
                </div>
                <div class='box_content wc_normalize_height'>
                    ...
                    <h1 class='title'><a title='Ver Imóvel No Site' target='_blank' href='" . BASE . "/imovel/{$realty_name}'>" . Check::Chars($realty_title, 40) . "</a> <b class='font_yellow'> " . ($realty_price ? "R$&nbsp;" . number_format($realty_price, 2, ',', '.') : 'À Combinar') . "</b></h1>
                ...
                <div class='box_actions'>
                    <a title='Editar Imóvel' href='dashboard.php?wc=imobi/create&id={$realty_id}' class='post_single_center icon-pencil btn btn_darkblue'>Editar</a>
                    <span title='Excluir Imóvel' class='j_delete_action icon-cancel-circle btn btn_red' callback='Properties' callback_action='delete' id='{$realty_id}'>Excluir</span>
                </div>
            </article>";
        endforeach;
