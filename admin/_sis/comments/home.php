...
<header class="dashboard_header">
    ...
    <div class="dashboard_header_search">
        <a title="Recarregar Comentários" href="dashboard.php?wc=comments/home" class="btn_header btn_aquablue icon-undo2">Voltar</a>
...
<div class="dashboard_content">
    <section class="box box100">
        ...
        <div class="panel">
            ...
            if (!$Read->getResult()):
                $Pager->ReturnPage();
                echo Erro("<span class='icon-info al_center'>Olá {$_SESSION['userLogin']['user_name']}. Ainda Não Existem Comentários. Mas Isso Não Deve Demorar!</span>", E_USER_NOTICE);
            else:

                //STATUS
                $SupportStatus = [1 => 'Respondido', 2 => 'Em Aberto', 3 => 'Moderar'];
                $SupportStatusClass = [1 => 'btn_green icon-bubbles3', 2 => 'btn_red icon-bubble2', 3 => 'btn_aquablue icon-bubble2'];
                ...
                foreach ($Read->getResult() as $Comm):
                    ...
                    //RANK
                    $Stars = str_repeat("<span class='icon-star-full icon-notext font_aquablue review'></span>", $Comm['rank']);
                    ?>
                    <article class="ead_support_single single_comment js-rel-to" id="<?= $Comm['id']; ?>">
                        <h1 class="row">
                            <a class="a" target="_blank" href='<?= BASE; ?>/<?= $Link; ?>#<?= $Comm['id']; ?>' title='Ver Fórum Na Aula'>#<?= str_pad($Comm['id'], 4, 0, 0); ?> - <?= $Title; ?></a>
                        </h1><p class="row icon-user">
                        ...
                        </p><p class="row icon-calendar">
                            <?= date('d/m/Y H\hi', strtotime(($Comm['interact'] ? $Comm['interact'] : $Comm['created']))); ?>
                        </p><p class="row btn_support">
                            <a title="Responder Ticket" href="dashboard.php?wc=comments/comment_response&id=<?= $Comm['id']; ?>" class="btn <?= $SupportStatusClass[$Comm['status']]; ?>"><?= $SupportStatus[$Comm['status']]; ?></a>
                        </p>
                    </article>
