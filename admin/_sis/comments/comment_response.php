...
<header class="dashboard_header">
    ...
    <div class="dashboard_header_search">
        <a title="Recarregar Comentários" href="dashboard.php?wc=comments/home" class="btn_header btn_aquablue icon-spinner11 icon-notext"></a>
        <a title="Ver Comentários" href="dashboard.php?wc=comments/comment_response" class="btn_header btn_darkaquablue icon-eye">Ver Comentários</a>
    </div>
</header>
...
<div class="dashboard_content">
    <section class="box box100">
        <div class="panel" style="padding: 30px;">
            ...
            if (!$Read->getResult()):
                echo "<div class='trigger trigger_success trigger_none al_center icon-heart font_medium'>Olá {$_SESSION['userLogin']['user_name']}, Não Existem Mais Comentários Em Aberto. :)</div><div class='clear'></div>";
            else:
                ...
                //STATUS
                $TicketStatus = ($status == 2 ? "<span class='status bar_red radius'>Em Aberto</span>" : ($status == 1 ? "<span class='status bar_red radius'>Respondido</span>" : ($status == 3 ? "<span class='status bar_green radius'>Concluído</span>" : '')));
                ...
                <article class="ead_support_response" id="<?= $id; ?>" style="margin-bottom: 20px;">
                    <div class="ead_support_response_avatar">
                        ...
                        <div class="ead_support_response_actions">
                            ...
                            if (!$Read->getResult()):
                                ?>
                                <a style="color: #fff; font-weight: bold;" href='#<?= $Comm['id']; ?>' class='btn btn_aquablue wc_comment_action icon-heart' id="<?= $Comm['id']; ?>" rel='<?= $Comm['id']; ?>' action='like' href='Gostei do Comentário' title='Gostei do Comentário'>GOSTEI :)</a>
                                <?php
                            endif;
                            ?>

                            <span class="btn btn_darkaquablue icon-pencil2 j_comment_action" data-action='comment_edit' id="<?= $id; ?>">Editar</span>
                            <span rel='ead_support_response' class='j_delete_action icon-cancel-circle btn btn_red m_mobile' callback='Comments' callback_action='remove_comment' id='<?= $id; ?>'>Apagar</span>
                            
                            <?php
                            if ($status == 2):
                                ?>
                                <span class="btn btn_green icon-checkmark j_comment_action ead_support_finish m_mobile" data-action='comment_completed' id="<?= $id; ?>">Concluir</span>
                                <?php
                            endif;
                            ?>
                                    ...
                                    echo "<article class='ead_support_response ead_support_response_reply reply' id='{$ResponseReply['id']}'>
                                                <div class='ead_support_response_avatar'>
                                                    ...
                                                    <div class='ead_support_response_actions'>
                                                        ...
                                                        <span class='btn btn_aquablue icon-pencil2 center m_mobile j_ead_support_action' data-action='ead_support_reply_edit' id='{$ResponseReply['id']}'>Editar</span>
                                                    </div>
                                                </div>       
                                                <div class='ead_support_response_edit_modal reply'>
                                                    <form name='class_adda' action='' method='post' enctype='multipart/form-data'>
                                                        ...
                                                        <div class='wc_actions' style='margin-top: 15px;'>
                                                            <button class='btn btn_aquablue icon-pencil2'>ATUALIZAR RESPOSTA <img class='form_load none' style='margin-left: 6px; margin-bottom: 9px;' alt='Enviando Requisição!' title='Enviando Requisição!' src='_img/load.svg'/></button>
                                                        </div>
                                                    </form>
                                                </div>
                                        </article>";
                        ...
                        <?php
                        if ($rank):
                            $ReviewPositive = '<span class="icon-star-full icon-notext font_aquablue"></span>';
                    ...
                    <div class="ead_support_response_edit_modal response">
                        <form name="class_edit" action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="wc_actions" style="margin-top: 15px;">
                                <button title="ATUALIZAR" class="btn_big btn_aquablue icon-share">ATUALIZAR <img class="form_load none" style="margin-left: 6px; margin-bottom: 9px;" alt="Enviando Requisição!" title="Enviando Requisição!" src="_img/load.svg"/></button></button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="ead_support_response_edit_modal remove">
                        <form name="class_add" action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="wc_actions" style="margin-top: 15px;">
                                <button title="NOTIFICAR E EXCLUIR" class="btn_big btn_red icon-cross">NOTIFICAR E EXCLUIR <img class="form_load none" style="margin-left: 6px; margin-bottom: 9px;" alt="Enviando Requisição!" title="Enviando Requisição!" src="_img/load.svg"/></button>
                            </div>
                        </form>
                    </div>
                </article>
                ...
                <form name="class_add" class="ead_support_response_form" action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="wc_actions" style="margin-top: 25px;">
                        <button title="RESPONDER" style="padding: 10px 25px; margin-right: 10px;" class="btn_big btn_aquablue icon-bubble">RESPONDER <img class="form_load none" style="margin-left: 6px; margin-bottom: 9px;" alt="Enviando Requisição!" title="Enviando Requisição!" src="_img/load.svg"/></button>
                        <a title="PRÓXIMO COMENTÁRIO" class="btn_big btn_darkaquablue icon-arrow-right m_mobile" style="padding: 10px 25px;" href="dashboard.php?wc=comments/comment_response" title="Próximo Comentário Pendente">PRÓXIMO COMENTÁRIO</a>
                    </div>
                </form>
            </div>
        <?php
