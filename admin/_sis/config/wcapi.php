...
<div class="dashboard_content">
    <div class="wc_api_new">
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <input required type="text" name="api_key" value="" placeholder="API Key, Ex: www.workcontrol.com.br"/><button title="Criar APP" class="btn btn_darkaquablue">Criar APP <img class="form_load none" style="margin-left: 6px; margin-bottom: 9px;" alt="Enviando Requisição!" title="Enviando Requisição!" src="_img/load.svg"/></button>
        </form>
            ...
            <article class="box box50 wc_api_app api_single" id="<?= $api_id; ?>">
                ...
                <div class="box_content">
                    ...
                    <div class="wc_api_app_actions">
                        <span rel="api_single" style="<?= ($api_status == 1 ? '' : 'display: none;'); ?>" callback="Api" callback_action="inactive" class="jwc_active_action jwc_inactive icon-checkmark icon-notext btn_header btn_green" id="<?= $api_id; ?>"></span>
                        <span rel="api_single" style="<?= ($api_status == 1 ? 'display: none;' : ''); ?>" callback="Api" callback_action="active" class="jwc_active_action jwc_active icon-warning icon-notext btn_header btn_yellow" id="<?= $api_id; ?>"></span>
                        <span rel="api_single" class="j_delete_action icon-notext icon-cancel-circle btn_header btn_red" callback="Api" callback_action="delete" id="<?= $api_id; ?>"></span>
                    </div>
                </div>
            </article>
            <?php
