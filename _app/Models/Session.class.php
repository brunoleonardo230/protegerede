...
//        die;
        ...
        $this->getDevice();
        $this->Session['online_device'] = $this->Device;
        ...
        /*
         *  ====== ATENÇÃO =========
         * Se quiser salvar apenas uma sessão para cada IP Retire o comentario do código abaixo e elimine
         * as 3 linhas acima onde é executado o Create e setado a Sessão
         * O código abaixo faz uma leitura no Banco para verificar se o IP em questão ja possui uma sessão ativa.
         */

        $Read = new Read();
        $Read->FullRead("SELECT online_id FROM " . DB_VIEWS_ONLINE . " WHERE online_ip = '{$this->Session['online_ip']}'");
        if (!$Read->getResult()):
            $Create = new Create;
            $Create->ExeCreate(DB_VIEWS_ONLINE, $this->Session);
            $_SESSION['userOnline'] = $Create->getResult();
    ...
    private function getDevice() {
        ...
        if (strpos($this->Session['online_agent'], "iPhone")):
            $this->Device = "Iphone";
        elseif (strpos($this->Session['online_agent'], "iPad")):
            $this->Device = "Ipad";
        elseif (strpos($this->Session['online_agent'], "Android")):
            $this->Device = "Android";
        elseif (strpos($this->Session['online_agent'], "Windows")):
            $this->Device = "Desktop";
        else:
            $this->Device = "Desconhecido";
        endif;
    }
